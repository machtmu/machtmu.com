---
title: Hot Fire Test - September 28th, 2024
---

# GAR-E Hot Fire Test - September 28th, 2024

This was GAR-E's successful final hot-fire test.

## Test Video

<figure style="margin:1rem auto 2rem; width:100%; max-width:1000px; text-align:center;">
  <video controls playsinline preload="metadata" poster="hotfire-poster.webp" style="display:block; width:100%; height:auto; border-radius:8px; background:#000;">
    <source src="hotfire.mp4" type="video/mp4">
    Your browser does not support embedded video.
  </video>
  <figcaption style="font-size:0.9rem; color:#888; margin-top:0.5rem;">
    GAR-E hot-fire test on September 28th, 2024. <a href="hotfire.mp4" download>Download video</a>
  </figcaption>
</figure>

## Test Photos

<div style="display:grid; grid-template-columns:repeat(auto-fit,minmax(min(100%,280px),1fr)); gap:1rem; margin:1rem 0 2rem;">
  <figure style="margin:0; width:100%;">
    <a href="https://drive.google.com/file/d/1g2gI_7q6t6NCbkoR9taHEGbFPFZ7KfOv/view" target="_blank" rel="noopener" style="display:block; width:100%; aspect-ratio:4/3; overflow:hidden; border-radius:8px; background:#111;">
      <img src="test-setup.webp" alt="GAR-E test stand setup on September 28th, 2024" loading="lazy" decoding="async" style="width:100%; height:100%; display:block; object-fit:contain;">
    </a>
    <figcaption style="font-size:0.9rem; color:#888; margin-top:0.5rem; text-align:center;">Test stand setup</figcaption>
  </figure>

  <figure style="margin:0; width:100%;">
    <a href="https://drive.google.com/file/d/1P9YZKVELce7l6HkKTBfkXMyD0YKNpm4q/view" target="_blank" rel="noopener" style="display:block; width:100%; aspect-ratio:4/3; overflow:hidden; border-radius:8px; background:#111;">
      <img src="controls.webp" alt="GAR-E controls and instrumentation on September 28th, 2024" loading="lazy" decoding="async" style="width:100%; height:100%; display:block; object-fit:contain;">
    </a>
    <figcaption style="font-size:0.9rem; color:#888; margin-top:0.5rem; text-align:center;">Controls and instrumentation</figcaption>
  </figure>

  <figure style="margin:0; width:100%;">
    <a href="https://drive.google.com/file/d/1hNGlA3DMwiSr4T6LqzP1YOVJ0UJCDvyh/view" target="_blank" rel="noopener" style="display:block; width:100%; aspect-ratio:4/3; overflow:hidden; border-radius:8px; background:#111;">
      <img src="test-fire.webp" alt="GAR-E hot-fire test on September 28th, 2024" loading="lazy" decoding="async" style="width:100%; height:100%; display:block; object-fit:contain;">
    </a>
    <figcaption style="font-size:0.9rem; color:#888; margin-top:0.5rem; text-align:center;">Hot-fire test</figcaption>
  </figure>

  <figure style="margin:0; width:100%;">
    <a href="https://drive.google.com/file/d/1popNp6ublV6EhHp_qlU8lfbQTN74v3Ul/view" target="_blank" rel="noopener" style="display:block; width:100%; aspect-ratio:4/3; overflow:hidden; border-radius:8px; background:#111;">
      <img src="engine-work.webp" alt="GAR-E engine preparation on September 28th, 2024" loading="lazy" decoding="async" style="width:100%; height:100%; display:block; object-fit:contain;">
    </a>
    <figcaption style="font-size:0.9rem; color:#888; margin-top:0.5rem; text-align:center;">Engine preparation</figcaption>
  </figure>
</div>

## Team Photo

<figure style="margin:2rem auto; display:flex; flex-direction:column; align-items:center; width:100%; text-align:center;">
  <a href="https://drive.google.com/file/d/1hXNXq7gPc806abhY1mTzlJ7LFzDUH-dA/view" target="_blank" rel="noopener" style="display:block; width:100%; max-width:1000px;">
    <img src="team.jpg" alt="GAR-E team working around the test stand during the September 28th, 2024 hot-fire test" loading="lazy" decoding="async" style="width:100%; height:auto; border-radius:8px; display:block; object-fit:cover;">
  </a>
  <figcaption style="font-size:0.9rem; color:#888; margin-top:0.5rem;">The GAR-E team at work during the September 28th, 2024 hot-fire test.</figcaption>
</figure>
